...
public class SkinnedTransformHider : ITransformHider
{
    ...
    private const int FrameInitCount = 2;
    ...
    // alt hider
    private readonly Stack<Transform[]> myStack = new();
    private readonly Transform[] myHideBones;
    ...
    public SkinnedTransformHider(SkinnedMeshRenderer renderer, IReadOnlyDictionary<Transform, FPRExclusion2> exclusions)
    {
        ...
        SubTask.FindExclusionVertList(renderer, exclusions, ref myHideBones);

        foreach (var exclusion in exclusions)
        {
            FPRExclusion2 fprExclusion = exclusion.Value;
    ...
    private class SubTask : IFPRExclusionTask
    {
        ...
        public SubTask(SkinnedTransformHider parent, FPRExclusion2 exclusion, List<int> exclusionVerts)
        {
            _parent = parent;
        ...
        public static void FindExclusionVertList(SkinnedMeshRenderer renderer, IReadOnlyDictionary<Transform, FPRExclusion2> exclusions, ref Transform[] hideTransforms) 
        {
            ...
            for (int i = 0; i < renderer.bones.Length; i++)
            {
                ...
                if (!exclusions.TryGetValue(renderer.bones[i], out FPRExclusion2 _)) 
                    continue;
                
                weights.Add(i);
            }
            
            hideTransforms = new Transform[renderer.bones.Length];
            for (int i = 0; i < hideTransforms.Length; i++)
            {
                if (weights.Contains(i))
                    hideTransforms[i] = TransformHiderManager.headZeroCopy;
                else
                    hideTransforms[i] = renderer.bones[i];
            }
