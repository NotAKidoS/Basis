...
public class MeshTransformHider : ITransformHider, IFPRExclusionTask
{
    ...
    public MeshTransformHider(MeshRenderer renderer, IReadOnlyDictionary<Transform, FPRExclusion2> exclusions)
    {
        Transform rootBone = renderer.transform;
        
        // if no key found, dispose
        if (!exclusions.TryGetValue(rootBone, out FPRExclusion2 exclusion))
        {
            Dispose();
            return;
        }
